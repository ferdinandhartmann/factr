...
from utils_data_process import downsample_data, lowpass_filter, medianfilter
...
def plot_joint_data(
    ...
    if all(k in data_dict for k in required_keys_2):
        ...
        if median_filter_torque:
            broadcaster_torques_med_filt = medianfilter(
                broadcaster_torques, kernel_size=median_filter_kernel_size_torque
            )
        else:
            broadcaster_torques_med_filt = broadcaster_torques
        if filter_torque:
            broadcaster_torques_filtered = lowpass_filter(
                np.array(broadcaster_torques_med_filt), cutoff_freq=10.0, fs=50.0
            )
        else:
            broadcaster_torques_filtered = broadcaster_torques_med_filt
        if downsample:
            broadcaster_torques_filtered, _ = downsample_data(
                broadcaster_torques_filtered, data_frequency, target_downsampling_freq
            )
        ...
        for i in range(7):
            ...
            (line1,) = ax1.plot(
                broadcaster_ts,
                broadcaster_torques[:, i],
                linewidth=1,
                label="Broadcasted Torque [Nm]",
                alpha=1.0,
                color="red",
            )
            if downsample:
                (line2,) = ax1.plot(
                    broadcaster_ts_downsampled,
                    broadcaster_torques_filtered[:, i],
                    linewidth=1,
                    label="Broadcasted Torque Filtered [Nm]",
                    alpha=1.0,
                    color="blue",
                )
            else:
            ...
            lines = [line1, line2]
            labels = [line_label.get_label() for line_label in lines]
            ax1.legend(lines, labels, loc="upper right", fontsize=8)
    ...
    if all(k in data_dict for k in required_keys_1):
        ...
        if filter_position:
            commanded_pos_filtered = lowpass_filter(
                np.array(commanded_pos_medfilt), cutoff_freq=cutoff_freq_position, fs=data_frequency
            )
        else:
            commanded_pos_filtered = commanded_pos_medfilt
        if downsample:
            commanded_pos_filtered, _ = downsample_data(
                commanded_pos_filtered, data_frequency, target_downsampling_freq
            )
        ...
        for i in range(7):
            ...
            (line1,) = ax1.plot(
                broadcaster_ts,
                broadcaster_states[:, i],
                linewidth=1.5,
                label="/franka_robot_state_broadcaster/measured_joint_states [rad]",
                alpha=1,
                color="darkgreen",
            )
            (line2,) = ax1.plot(
                ...
                commanded_pos[:, i],
                linewidth=1,
                label="Commanded Pos to controller (joint_trajectory) [rad]",
                alpha=1,
                color="blue",
            )
            if downsample:
                (line3,) = ax1.plot(
                    commanded_ts_downsampled,
                    commanded_pos_filtered[:, i],
                    linewidth=1,
                    label="Commanded Pos to controller smoothed (joint_trajectory) [rad]",
                    alpha=1.0,
                    color="red",
                )
            else:
...
def gaussian_2d_kernel(
    kernel_size: int, sigma: float, device: torch.device = torch.device("cpu"), dtype: torch.dtype = torch.float32
) -> torch.Tensor:
...
def visualize_curriculum_steps(pkl_data, output_dir, topic_name="/realsense/front/im"):
    ...
    if "data" not in first_image or first_image["encoding"] != "rgb8":
        print("❌ First image is not 'rgb8' or missing data key. Skipping curriculum visualization.")
        return
...
if __name__ == "__main__":
    ...
    print("🎯 Single file visualization complete!")

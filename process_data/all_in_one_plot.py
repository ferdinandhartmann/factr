...
def plot_all_traj_in_one_plot(data_path, output_dir, step=1):
    ...
    for pkl_file in pkl_files[1:]:
        try:
            ...
            x_pos, (meas_pos_t, cmd_pos_t, obs_pos_t) = trim_minlen([meas_pos, cmd_pos, obs_pos])

            # Group 2: torques (broadcaster / observed)
            x_tq, (brd_torq_t, obs_torq_t) = trim_minlen([brd_torq, obs_torq])
            ...
            if any(v is not None for v in [meas_pos_t, cmd_pos_t, obs_pos_t, brd_torq_t, obs_torq_t]):
                entries.append({
                    'name': pkl_file.stem,
                    'x_pos': x_pos,
                    'meas_pos': meas_pos_t,
...
if __name__ == '__main__':
    ...
    dataset_folder = Path("/home/ferdinand/factr/process_data/data_to_process/20251024/data/")

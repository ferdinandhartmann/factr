...
parser = argparse.ArgumentParser(description="Run Posterior Inference Check")
parser.add_argument("--model_name", type=str, help="Name of the model (folder name in checkpoints)")
args = parser.parse_args()

model_name = args.model_name  # 引数から受け取った値を使用
...
SAVE_DIR = SAVE_DIR_ROOT / f"POSTERIOR_GT_CHECK_{MODE}"
...
def run_posterior_inference(policy, imgs, obs, target_action, num_samples=10):
    """
    GTアクションを入力としてPosteriorからzをサンプリングし、
    アクションを再構成する（事後分布の確認用）。

    Args:
        policy: TransformerAgentインスタンス
        imgs, obs: 観測データ
        target_action: 正解アクション (B, Chunk, Dim) または (B, Flat)
        num_samples: 1つの入力に対するサンプリング数

    Returns:
        raw_samples_np: (B, num_samples, Chunk, Dim)
        pred_mean: (B, Chunk, Dim)
        gt_input: (B, Chunk, Dim)
    """

    action_samples = policy.get_actions_pos(
        imgs, obs, target_action, num_samples=num_samples, sample=True
    )  # Output: (B, num_samples, Chunk, Dim)

    raw_samples_np = action_samples.detach().cpu().numpy()  # numpy化
    pred_mean_np = raw_samples_np.mean(axis=1)  # (B, Chunk, Dim) num_sampleをつぶし平均化

    # 3. GTデータの整形 (リターン用)関係なし
...
sanity_check = cfg.agent.get("sanity_check_posterior", False)

if sanity_check:
    print("\n" + "=" * 60)
    print("⚠️  [GT-Only Mode] Detected sanity_check_posterior=True in Config")
    print("    Posterior input will be TARGET ACTION ONLY.")
    print("=" * 60 + "\n")
    cfg.agent.sanity_check_posterior = True
else:
    print("\n" + "=" * 60)
    print("ℹ️  [Standard Mode] sanity_check_posterior=False")
    print("    Posterior input will be CONTEXT + TARGET ACTION.")
    print("=" * 60 + "\n")
...
for episode_name in episode_names:
    ...
    cvae_samples_list = []
    input_gt_list = []  # ★入力GT確認用

    image_history = deque(maxlen=img_chunk)

    print(f"Running Posterior Check on {episode_name}...")
    for i in tqdm(range(len(image_obs)), leave=False):
        ...
        if current_gt_chunk.shape[0] < chunk_size:
            ...
            last_val = current_gt_chunk[-1].unsqueeze(0)
            padding = last_val.repeat(pad_len, 1)
            current_gt_chunk = torch.cat([current_gt_chunk, padding], dim=0)

        current_gt_chunk = current_gt_chunk.unsqueeze(0)  # (1, Chunk, Dim)

        with torch.no_grad():
            raw_samples, pred_mean, recovered_gt = run_posterior_inference(
                policy, {"cam0": input_img_tensor}, torque_norm, target_action=current_gt_chunk, num_samples=NUM_SAMPLES
            )
            ...
            input_gt_list.append(recovered_gt[0])

    # --- Plotting Preparation ---
    pred_actions_mean = np.array(pred_actions_mean_list)  # (Time, Chunk, Dim)
    input_gt_arr = np.array(input_gt_list)  # (Time, Chunk, Dim)
    ...
    # -----------------------------------------------------------
    # Plot 1: Reconstruction Check (All 30 steps visualized)
    # -----------------------------------------------------------
    ...
    for d in range(7):
        ...
        # # --- 2. Recovered Input GT (緑色) ---
        # for step_i in range(chunk_size_actual):
        #     if step_i == 0:
        #         ax.plot(t + step_i, input_gt_arr[:, step_i, d],
        #                 color='green', linewidth=1.5, alpha=0.8, linestyle=":", label="Recovered Input (Flatten->Reshape)")

        # --- 3. Posterior Samples (水色) ---
        plot_interval = 4

        if samples_arr is not None:
            for step_i in range(chunk_size_actual):
                for s_idx in range(NUM_SAMPLES):
                    lbl = "Posterior Sample" if (step_i == 0 and s_idx == 0) else None
                    ax.plot(
        ...
        #                 color='blue', linewidth=1.2, alpha=0.9, label="Posterior Mean (Step 0)")
        #     else:
        #         ax.plot(t + step_i, pred_actions_mean[:, step_i, d],
    ...
    save_path_act = episode_save_dir / "posterior_reconstruction_with_input_check_per4.png"
    plt.savefig(save_path_act, dpi=150)
    plt.close(fig1)

    print(f"✅ Saved results for {episode_name}")

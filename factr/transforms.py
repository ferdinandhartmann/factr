...
class _MediumAug(nn.Module):
    ...
    def forward(self, x):
        ...
        arange = torch.linspace(-1.0 + eps, 1.0 - eps, h + 2 * self.pad, device=x.device, dtype=x.dtype)[: self.size]
...
def get_transform_by_name(name, size=224):
    ...
    if name == "basic":
        return transforms.Compose(
            [
                transforms.RandomResizedCrop(size=size, scale=(0.2, 1.0), antialias=False),
                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            ]
        )
    if name == "medium":
        ...
        return transforms.Compose(
            [
                transforms.RandomResizedCrop(
                    size=size,
                    scale=(0.9, 1.0),
                    antialias=False,  # randomly crop with a factor between 0.9 and 1
                ),
        ...
        return transforms.Compose(
            [
                transforms.RandomResizedCrop(
                    size=size,
                    scale=(0.98, 1.0),
                    antialias=False,  # randomly crop with a factor between 0.98 and 1
                ),
                transforms.GaussianBlur(kernel_size=kernel_size),  # here gaussian blurr added
                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
            ]
    ...
    if name == "hamer":
        ...
        return transforms.Compose(
            [
                ...
                transforms.RandomResizedCrop(size=(256, 196), scale=(0.9, 1.0), antialias=False),
                transforms.GaussianBlur(kernel_size=kernel_size),

...
class LowdimStiffnessCVAEAgent(nn.Module):
    ...
    def __init__(
        ...
        if self.include_tracking_error:
            ...
            num_tokens = 5 if self.use_cls_token else 4
        ...
        self.gripper_encoder = make_group_encoder(1)
        ...
        if self.include_tracking_error:
            context_tokens = 5 if self.use_cls_token else 4
        else:
            context_tokens = 4 if self.use_cls_token else 3
        context_dim = context_tokens * token_dim
    ...
    def _build_context_tokens(self, obs, class_labels):
        ...
        gripper = obs[:, :, self.state_slices["gripper"]].reshape(batch_size, -1)
        ...
        gripper_token = self.gripper_encoder(gripper)
        ...
        token_list.extend([pose_token, vel_token, gripper_token])
    ...
    def _build_z_context(self, context_tokens):
        ...
        gripper_token = context_tokens[:, start + 2]
        if self.include_tracking_error:
            track_token = context_tokens[:, start + 3]
            if self.use_cls_token:
                cls_token = context_tokens[:, 0]
                return torch.cat([cls_token, pose_token, vel_token, gripper_token, track_token], dim=-1)
            return torch.cat([pose_token, vel_token, gripper_token, track_token], dim=-1)

        if self.use_cls_token:
            cls_token = context_tokens[:, 0]
            return torch.cat([cls_token, pose_token, vel_token, gripper_token], dim=-1)
        return torch.cat([pose_token, vel_token, gripper_token], dim=-1)

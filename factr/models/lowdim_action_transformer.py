...
class LowdimStiffnessCVAEAgent(nn.Module):
    ...
    def __init__(
        ...
        action_flat_dim = self._ac_chunk * self._ac_dim
        self.posterior_action_encoder = nn.Sequential(
        ...
        self.posterior_backbone = nn.Sequential(
            nn.LayerNorm(context_dim + hidden_dim),
            nn.Linear(context_dim + hidden_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.GELU(),
        )
        self.posterior_mu = nn.Linear(hidden_dim, d_z)
        self.posterior_logvar = nn.Linear(hidden_dim, d_z)
        nn.init.constant_(self.posterior_logvar.bias, -3.0)
    ...
    def _posterior(self, z_context, target_actions):
        action_feat = self.posterior_action_encoder(target_actions.reshape(target_actions.shape[0], -1))
        h = self.posterior_backbone(torch.cat([z_context, action_feat], dim=-1))
        mu = self.posterior_mu(h)
        logvar = self.posterior_logvar(h)
        return mu, logvar
    ...
    def _decode_actions(self, context_tokens, z):
        ...
        target_queries = target_queries + z_query_bias # z is injected into every action query, not only as a memory token
    ...
    def forward(self, imgs, obs, ac_flat, mask_flat, class_labels=None, **kwargs):
        ...
        mu_q, logvar_q = self._posterior(z_context, target_actions)
        ...
        return {"total_loss": total_loss, "l1_loss": recon, "kl": kl}
    ...
    @torch.no_grad()
    def get_actions_prior(self, imgs, obs, class_labels=None, sample=True, num_samples=1, return_weights=False, **kwargs):
        del imgs, kwargs
        context_tokens = self._build_context_tokens(obs, class_labels=class_labels)
        z_context = self._build_z_context(context_tokens)
        mu_p, logvar_p = self._prior(z_context)
    ...
    def get_actions_pos(self, imgs, obs, target_action, class_labels=None, num_samples=1, sample=True, **kwargs):
        ...
        mu_q, logvar_q = self._posterior(z_context, target_action)
